# Database-Tables anlegen

Dieser Guide zeigt, wie Sie eigene Database-Tables mit SQLAlchemy-Modellen create.
...
2. **`__tablename__` definieren**: Tabellenname in Database
3. **Primary Key erforderlich**: Mindestens a Spalte als `primary_key=True`
4. **Type Hints verwenden**: `Mapped[type]` für alle Spalten

## 2. Example: Parameter-Table (tb_params)

Vollständiges Example aus vyra_base:
...
class Parameter(Base):
    """
    SQLAlchemy-Model für Modul-Parameter mit Metadaten.
...
new_param = {
    ...
    "description": "Maximale Wartezeit für Connections",
    "min_value": "1",
    "max_value": "300"
}
...
class User(Base):
    """User-Model mit Authentifizierung."""
    __tablename__ = "user"
...
│       └── application/
│           └── component.py     # Verwendet die Models
```
...
### Neue Table hinzufügen

```python
# 1. Model in models/ create
# 2. In models/__init__.py importieren
# 3. Database neu öffnen (erstellt Tables automatisch)

db = DbAccess(db_path="/workspace/storage/database/module.db")
await db.open()  # Erstellt fehlende Tables
```

### Spalte hinzufügen (Breaking Change!)

```python
# WARNUNG: Änderungen an existierenden Tables erfordern Migration!
# Für Produktion: Alembic verwenden

# Example: Neue Spalte mit Default-Wert
class User(Base):
    __tablename__ = "user"
...
# Default-Werte für Optional Felder
...
class BadTable(Base):
    ...
    name: Mapped[str] = mapped_column(String(100))  # Error!
...
### Table wird nicht erstellt

```python
# Prüfen, ob Model in Base registriert ist
from vyra_base.storage.tb_base import Base
print(Base.metadata.tables.keys())
# Sollte Ihre Table enthalten

# Manuelle Tabellenerstellung
await db.create_tables()  # Erstellt alle registrierten Tables
```

### "Table already exists" Error

```python
# Table existiert bereits in DB
...
# Existierende Database unterstützt SUSPENDED nicht automatisch!
# Lösung: Alembic Migration oder Spalte neu create
```
