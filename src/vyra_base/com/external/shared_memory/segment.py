...
class SharedMemorySegment:
    ...
    def __init__(
        ...
        self._transaction_support = None  # Lazy init
    ...
    def write(self, data: Any, message_type: MessageType = MessageType.REQUEST) -> bool:
        """
        Write data to shared memory segment (thread-safe).
        ...
        with self._thread_lock:
            return self._unsafe_write(data, message_type)
    
    def _unsafe_write(self, data: Any, message_type: MessageType = MessageType.REQUEST) -> bool:
        """
        Internal write implementation (not thread-safe, use write() instead).
    ...
    def read(self, timeout: float = 5.0) -> Tuple[MessageType, Any, int]:
        """
        Read data from shared memory segment (thread-safe).
        ...
        """
        with self._thread_lock:
            return self._unsafe_read(timeout)
    
    def _unsafe_read(self, timeout: float = 5.0) -> Tuple[MessageType, Any, int]:
        """
        Internal read implementation (not thread-safe, use read() instead).
    ...
    def transaction(self):
        ...
        Example:
            >>> segment = SharedMemorySegment("data", 1024, create=True)
            >>> with segment.transaction() as (current_value, commit):
            ...     new_value = current_value + 1
            ...     commit(new_value)
        """
        # Lazy init transaction support
        if self._transaction_support is None:
            try:
                ...
                self._transaction_support = TransactionalMemory(
                    get_global_safety_manager()
                )
            except ImportError:
                logger.warning("Transaction support not available (safety.py not found)")
                return self._dummy_transaction()
        
        return self._transaction_support.transaction(
            key=self.name,
            read_func=lambda: self._unsafe_read()[1],
            write_func=lambda val: self._unsafe_write(val)
        )
    
    def _dummy_transaction(self):
        """Dummy transaction for when safety module is not available."""
        from contextlib import contextmanager
        
        @contextmanager
        def dummy():
            current_value = self.read()[1]
            def commit(new_value):
                self.write(new_value)
            yield current_value, commit
