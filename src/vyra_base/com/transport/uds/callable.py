...
from vyra_base.com.transport.uds.socket import UnixSocket, UDS_SOCKET_DIR
...
class UDSCallable(VyraCallable):
    ...
    async def _initialize_server(self) -> bool:
        ...
        if self._socket is None:
            logger.warning("Could not start listener, no uds socket")
            raise
        else:
            await self._socket.listen(self._handle_request)  # type: ignore[arg-type]
    ...
    async def _initialize_client(self) -> bool:
        ...
        # Connect to server
        if self._socket is None:
            logger.warning("Could not connect client, no uds socket")
            raise
        else:
    ...
    async def call(self, request: Any, timeout: float = 5.0) -> Any:
        ...
        try:
            ...
            # Send request
            if self._socket is None:
                logger.warning("Could not send, no uds socket")
                raise
            else:
            ...
            # Receive response
            if self._socket is None:
                logger.warning("Could not receive, no uds socket")
            else:
