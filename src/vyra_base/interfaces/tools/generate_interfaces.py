...
# Proto well-known types that require an additional import statement.
# Maps proto type name → import line to inject into generated .proto files.
_PROTO_WELL_KNOWN_IMPORTS: Dict[str, str] = {
...
class InterfaceGenerator:
    ...
    def _collect_proto_imports(self, *field_lists: List[Dict[str, Any]]) -> List[str]:
        """
        Collect required proto import statements for well-known types used in a set of fields.

        Scans all provided field lists for datatypes that map to Protobuf well-known
        types (e.g. ``google.protobuf.Struct``).  Returns the unique, sorted set of
        import lines that must be prepended to the generated ``.proto`` file.

        Args:
            *field_lists: Variable number of field lists (params, returns, feedback, …).

        Returns:
            List of import lines (e.g. ``['import "google/protobuf/struct.proto";']``),
            empty when no well-known types are used.
        """
        needed: set[str] = set()
        for fields in field_lists:
            for field in fields:
                dt = field.get("datatype", "")
                proto_type = self._map_to_proto_type(dt)
                imp = _PROTO_WELL_KNOWN_IMPORTS.get(proto_type)
                if imp:
                    needed.add(imp)
    ...
    def generate_field(self, field: Dict[str, Any], is_return: bool = False) -> str:
        ...
        # Add comment with description; object fields always get the serialisation hint
        description = field.get("description", "")
        base_dt = datatype.rstrip("[]")
        if not description and base_dt == "object":
            description = "JSON-serialised dict (arbitrary key-value payload)"
        if description:
    ...
    def generate_proto_msg(self, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        extra_imports = self._collect_proto_imports(returns)
        lines = [
            'syntax = "proto3";',
            "",
        ]
    ...
    def generate_proto_srv(self, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        extra_imports = self._collect_proto_imports(params, returns)
        lines = [
            'syntax = "proto3";',
            "",
        ]
    ...
    def generate_combined_proto(self, metadata_entries: List[Dict[str, Any]], proto_filename: str) -> Optional[str]:
        ...
        # Collect imports needed across all entries in this combined proto
        all_fields: List[Dict[str, Any]] = []
        for _m in metadata_entries:
            all_fields.extend(_m.get("params", []))
            all_fields.extend(_m.get("returns", []))
        extra_imports = self._collect_proto_imports(all_fields)
    ...
    def generate_proto_action(self, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        extra_imports = self._collect_proto_imports(params, returns, feedback)
        lines = [
            'syntax = "proto3";',
            "",
        ]
