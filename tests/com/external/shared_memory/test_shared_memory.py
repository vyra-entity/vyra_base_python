...
# Skip entire module if shared_memory transport is not implemented
SharedMemoryProvider = pytest.importorskip(
    "vyra_base.com.transport.shared_memory.provider",
    reason="SharedMemory transport not implemented",
...
class TestSharedMemoryProvider:
    ...
    async def test_create_server(self, provider):
    ...
    async def test_create_publisher(self, provider):
    ...
    async def test_multiple_callables(self, provider):
        ...
        callable1 = await provider.create_server("service1")
        callable2 = await provider.create_server("service2")
    ...
    async def test_namespace_isolation(self):
        ...
        callable1 = await provider1.create_server("test")
        callable2 = await provider2.create_server("test")
...
class TestSharedMemoryCallable:
    ...
    async def test_callable_request_response(self):
        ...
        server_callable = await provider.create_server("echo_service")
...
class TestSharedMemorySpeaker:
    ...
    async def test_speaker_publish(self):
        ...
        speaker = await provider.create_publisher("status_topic")
